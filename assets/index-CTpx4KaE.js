(function(){const o=document.createElement("link").relList;if(o&&o.supports&&o.supports("modulepreload"))return;for(const t of document.querySelectorAll('link[rel="modulepreload"]'))n(t);new MutationObserver(t=>{for(const s of t)if(s.type==="childList")for(const a of s.addedNodes)a.tagName==="LINK"&&a.rel==="modulepreload"&&n(a)}).observe(document,{childList:!0,subtree:!0});function r(t){const s={};return t.integrity&&(s.integrity=t.integrity),t.referrerPolicy&&(s.referrerPolicy=t.referrerPolicy),t.crossOrigin==="use-credentials"?s.credentials="include":t.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function n(t){if(t.ep)return;t.ep=!0;const s=r(t);fetch(t.href,s)}})();const L="https://back-proyecto-10-mu.vercel.app/",$=()=>{const e=document.querySelector("body");e.className="",e.classList.add("body-newRegister");const o=document.querySelector("main"),r=`
    ...
    `;o.innerHTML=r;const n=document.querySelector("#newRegisterForm"),t=document.querySelector("#error-message"),s=async(a,i,d)=>{const u={method:"POST",body:JSON.stringify({userName:a,password:i,email:d}),headers:{"Content-Type":"application/json"}};try{const l=await fetch(`${L}/users/registro`,u);if(!l.ok){l.status===400?t.textContent="El nombre de usuario ya existe. Por favor, elija otro.":console.error("Error en la respuesta del servidor:",l.statusText);return}t&&(t.textContent="");const m=await l.json();console.log("Respuesta del servidor:",m),t.textContent="Usuario creado correctamente.",setTimeout(()=>{g()},8e3)}catch(l){console.error("Error en la solicitud:",l),t.textContent="Error en la solicitud. Inténtalo de nuevo más tarde."}};n.addEventListener("submit",function(a){a.preventDefault();const i=document.querySelector("#username").value,d=document.querySelector("#password").value,c=document.querySelector("#email").value;console.log("Username:",i),console.log("Password:",d),console.log("Email:",c),s(i,d,c)})},x=e=>`
        <div class="event-box">
    ...
    `,y="https://back-proyecto-10-mu.vercel.app/",q=()=>{const e=document.querySelector("body");e.className="",e.classList.add("body-newEvent");const o=document.querySelector("main"),r=`
    ...
    `;o.innerHTML=r;const n=localStorage.getItem("token"),t=document.querySelector("#newEventForm"),s=async()=>{try{const i=await fetch(`${y}/events/events`);if(!i.ok)throw new Error("Error al obtener los eventos");return await i.json()}catch(i){return console.error("Error al obtener los eventos:",i),[]}},a=async(i,d,c,u,l)=>{const f={method:"POST",body:JSON.stringify({name:i,date:d,time:c,location:u,description:l}),headers:{"Content-Type":"application/json",Authorization:`Bearer ${n}`}};try{const v=await fetch(`${y}/events/nuevoEvento`,f);if(!v.ok){console.error("Error en la respuesta del servidor:",v.statusText);return}const w=document.querySelector("#error-message"),E=await v.json();console.log("Respuesta del servidor:",E),w.textContent="El evento se ha creado correctamente.";const S=await s();setTimeout(()=>{b(S)},8e3)}catch(v){console.error("Error en la solicitud:",v)}};t.addEventListener("submit",function(i){i.preventDefault();const d=document.querySelector("#event-name").value,c=document.querySelector("#description").value,u=document.querySelector("#date").value,l=document.querySelector("#time").value,m=document.querySelector("#location").value;console.log("Name:",d),console.log("Description:",c),console.log("Date:",u),console.log("Time:",l),console.log("Location:",m),a(d,u,l,m,c)})},k=()=>{const e=document.querySelector("header");e.innerHTML=`
        <div class="header-btns">
            <button type="submit" class="btn btn-header" id="new-event-link">Crear nuevo evento</button>
        </div>
    `,document.querySelector("#new-event-link").addEventListener("click",r=>{r.preventDefault(),q();const n='<button class="btn btn-header" onclick="handleListEvents();">Volver</button>',t=document.querySelector(".header-btns");t.innerHTML+=n})},b=e=>{const o=document.querySelector("body");o.className="",o.classList.add("body-eventList");const r=document.querySelector("main"),t=`
            <section id="eventos">
        ...
        `;r.innerHTML=t,k()},h="https://back-proyecto-10-mu.vercel.app/",N=()=>{document.querySelector("body").classList.add("body-login");const o=document.querySelector("main"),r=`
    ...
    `;o.innerHTML=r,document.querySelector("#register-link").addEventListener("click",t=>{t.preventDefault(),$()})},P=async()=>{try{const e=await fetch(`${h}events/events'`);if(!e.ok)throw new Error("Error al obtener los eventos");return await e.json()}catch(e){return console.error("Error al obtener los eventos:",e),[]}},g=()=>{N();const e=document.querySelector("#loginForm"),o=document.querySelector("#username"),r=document.querySelector("#password"),n=document.querySelector("#error-message"),t=async(s,a)=>{const d={method:"POST",body:JSON.stringify({userName:s,password:a}),headers:{"Content-Type":"application/json"}};try{const c=await fetch(`${h}users/login`,d);if(c.status===400){n.textContent="Usuario o contraseña incorrectos";return}n&&n.remove();const u=await c.json();if(console.log("Respuesta del servidor:",u),u.token){localStorage.setItem("token",u.token);const l=await P();b(l)}else console.error("El token no está definido en la respuesta:",u),n.textContent="Error al iniciar sesión. Por favor, inténtalo de nuevo."}catch(c){console.error("Error en la petición:",c),n.textContent="Error en la conexión. Por favor, inténtalo de nuevo."}};e.addEventListener("submit",function(s){s.preventDefault(),t(o.value,r.value)})},T=e=>{const o=document.querySelector("body");o.className="",o.classList.add("body-eventPage");const r=document.querySelector("main"),n=`
